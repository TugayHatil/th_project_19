...
export class PlannerWorkspace extends Component {
    ...
    async moveTask(taskId, parentId, beforeId, afterId) {
        try {
            await this.orm.call("project.task", "planner_move_task", [taskId], {
                parent_id: parentId || false,
                before_task_id: beforeId || false,
                after_task_id: afterId || false,
            });
            const collapsed = this.state.collapsedIds;
            const reopenInspector = this.state.inspectorOpen;
            await this.loadProject(this.state.projectId);
            this.state.collapsedIds = collapsed;
            ...
            this.notification.add(error.data?.message || _t("The task could not be moved."), {
                type: "danger",
            });
        }
    }
    ...
    async onWbsDrop(ev) {
        ...
        const ownIndex = rows.findIndex((t) => t.id === taskId);
        // Dropping back onto its own slot changes nothing.
